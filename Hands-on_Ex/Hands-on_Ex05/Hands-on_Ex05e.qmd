...
subtitle: "Lesson 5e: Treemap Visualisation with R" 
author: "Victoria Neo"
date: 02/12/2024
...
# 5 [**Treemap Visualisation with R**](https://r4va.netlify.app/chap16)
...
### 5.2.1 Installing and loading the required libraries
...
### 5.2.3 **Data Wrangling and Manipulation**
...
Two key verbs of [**dplyr**](https://cran.r-project.org/web/packages/dplyr/vignettes/dplyr.html) package, namely: *group_by()* and *summarize()* will be used to perform these steps.
...
#### 5.2.3.1 **Grouped summaries without the *`pipe`***
...
#### 5.2.3.2 **Grouped summaries with the *`pipe`***

The code chunk below shows a more efficient way to tackle the same processes by using the [*pipe*](https://www.datacamp.com/community/tutorials/pipe-r-tutorial), %\>%:.
...
### 5.3.2 **Using the basic arguments**
...
### 5.3.3 **Working with *vColor* and *type* arguments**
...
::: {.kambox .kam data-latex="kam"}
Things to learn from the code chunk above used:
...
#### 5.3.4.2 **The “manual” type treemap**
...
#### 5.3.5.1 **Working with *algorithm* argument**
...
### 5.3.6 **Designing Treemap using treemapify Package**
...
#### 5.3.6.1 **Designing a basic treemap**
...
### 5.3.7 **Designing Interactive Treemap using d3treeR**

#### 5.3.7.1 **Installing *d3treeR* package**

This code chunk shows you how to install a R package which is not available in cran.
...
2.  Now you are ready to launch *d3treeR* package

```{r}
#library(d3treeR)
```

#### 5.3.5.2 **Designing An Interactive Treemap**
...
```{r}
d3tree(tm,rootname = "Singapore", width = "200%", height ="600px" )
```
