...
-   Plot a histogram to study the variation within BMI

-   Plot a notched boxplot to study the distribution of BMI and its differences in central tendency across dimensions
...
## 3.2 Looking at BMI through dimensions

### 3.2.1 Country dimension with Boxplot
...
bmi_data %>% 
  ...
  ggplot(aes(x = country, y = bmi_usd_price, fill = type, alpha = type)) + 
    ...
    geom_boxplot(width=0.4, color="black", alpha=0.8) +
...
### 3.2.2 Year dimension with Boxplot
...
3jklmk3k \## **using median bmi_usd_price as the baseline**
...
## 6.1 Simple Regression of selected years

At this point, we will estimate simple regressions using data for years 2002, 2011 and 2021 to model the relationship between the statistically significant correlated GDP per capita and the Big Mac Index price. We will then plot the data and add the corresponding estimated regression functions.

::: panel-tabset
## **2002**

```{r}
bmi_2002 <- subset(bmi_data, year == "2002")
bmi_2002_mod <- lm(bmi_usd_price ~ gdp_per_capita, data = bmi_2002)
coeftest(bmi_2002_mod, vcov. = vcovHC, type = "HC1")

```

```{r}
# plot the observations and add the estimated regression line for 2002 data
sr2002 <- ggplot(bmi_2002_mod, aes(x=gdp_per_capita, y=bmi_usd_price) ) +  
  geom_point(alpha = 0.5) +
  stat_poly_eq(use_label(c("eq", "R2"))) +
  geom_smooth(method = lm, linewidth = 0.2, color = "#008FD5", alpha = 0.3)  +
  coord_cartesian(xlim = c(0, 100000), ylim = c(0, 10)) +
 ylab('Big Mac Index price (US$) in 2002') + xlab('GDP per capita (US$) in 2002')  
...
```{r}
bmi_2011 <- subset(bmi_data, year == "2011")
bmi_2011_mod <- lm(bmi_usd_price ~ gdp_per_capita, data = bmi_2011)
coeftest(bmi_2011_mod, vcov. = vcovHC, type = "HC1")

```

```{r}
# plot the observations and add the estimated regression line for 2011 data
sr2011 <- ggplot(bmi_2011_mod, aes(x=gdp_per_capita, y=bmi_usd_price) ) +  
  geom_point(alpha = 0.5) +
  stat_poly_eq(use_label(c("eq", "R2"))) +
  geom_smooth(method = lm, linewidth = 0.2, color = "#008FD5", alpha = 0.3)  +
  coord_cartesian(xlim = c(0, 100000), ylim = c(0, 10)) +
 ylab('Big Mac Index price (US$) in 2011') + xlab('GDP per capita (US$) in 2011') 
...
```{r}
bmi_2021 <- subset(bmi_data, year == "2021")
bmi_2021_mod <- lm(bmi_usd_price ~ gdp_per_capita, data = bmi_2021)
coeftest(bmi_2021_mod, vcov. = vcovHC, type = "HC1")
```

```{r}
# plot the observations and add the estimated regression line for 2021 data
sr2021 <- ggplot(bmi_2021_mod, aes(x=gdp_per_capita, y=bmi_usd_price) ) +  
  geom_point(alpha = 0.5) +
  stat_poly_eq(use_label(c("eq", "R2"))) +
  geom_smooth(method = lm, linewidth = 0.2, color = "#008FD5", alpha = 0.3)  +
  coord_cartesian(xlim = c(0, 100000), ylim = c(0, 10)) +
 ylab('Big Mac Index price (US$) in 2021') + xlab('GDP per capita (US$) in 2021') 
...
patchwork <- sr2002 + sr2011 + sr2021
patchwork <- patchwork + 
  plot_annotation(
    title = "The higher the GDP per capita, the more likely you will have a higher Big Mac Index price.",
    subtitle = "Strong positive linear relationships between GDP per capita and Big Mac Index price for 2002, 2011 and 2022.",
    caption = "Data source: The Economist, World Bank",
...
In the three plots, each point represents observations of GDP per capita and BMI price in their respective year and the regression results indicate a positive relationship throughout the years. However, this cannot account for omitted unobservable factors that exist from country to country but can be assumed to be constant over the observed time period using panel data regression.
:::

## 6.2 Panel Data Regression

### 6.2.1 Choosing which model is appropriate
...
random <- plm(bmi_usd_price ~ gdp_per_capita+hdi, data=bmi_data, index=c("country", "year"), model="random")  #random model
...
## 6.3 Visualising Panel Data Regression with ExPanD
