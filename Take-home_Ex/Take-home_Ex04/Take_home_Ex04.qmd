...
```{r}
pacman::p_load(CGPfunctions, corrplot,
               d3treeR,
               DT,
               dplyr, ggExtra,
               ggHoriPlot,
...
-   What are the correlations between variables with BMI prices?
:::

## 5.2 Plotting a Correlation plot to see correlations between variables

```{r}
bmi.cor <- cor(bmi_data[, c(4:11)])
...
GDP per capita, inflation and Human Development Index are significantly correlated to Big Mac price Index. However, we cannot tell if there is a year/country effect - there is no information on the correlation even though though these variables are constant in the time dimension and across countries.
:::
