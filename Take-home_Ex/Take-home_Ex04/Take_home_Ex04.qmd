...
-   Plot a lollipop graph to observe the any patterns across dimensions
...
bmi_data %>% 
  ...
  mutate(type = ifelse(country %in% c("Russia", "Switzerland"), "Highlighted", "Normal")) %>%
...
The above boxplot also shows that there is variation within and between countries - there are some countries with greater variability e.g. Argentina and a number of countries with median BMI prices higher than the overall median BMI price such as Switzerland, Brazil and Canada while countries like Mexico and South Africa did not see much fluctuations in their BMI prices.
...
From the boxplot above, we can see that Big Mac prices have risen through the years - 2021 is the most expensive year for Big Mac prices. Despite the blip from 2013 - 2016 and 2018 - 2020, which saw falling Big Mac prices, as a whole, they have increased since 2002 which saw the lowest prices for Big Mac.
...
bmi_data %>% 
  ...
  newggslopegraph(year, bmi_usd_price, country,
                ...
                YTextSize = 2.5,
                LineThickness = .5,
                LineColor = c(rep("gray",8), "red", rep("gray",10), "#69b3a2", rep("gray",3),"#69b3a2",rep("gray",4)),
    WiderLabels = TRUE)
```
...
## 4.2 Looking at variations in patterns with `ggHoriPlot()` using mean as the baseline
...
The horizon plot showed that all countries were initially below the mean Big Mac price and all experienced some increase in price. China, Hong Kong, Korea, Singapore, Thailand and United States had a huge increase in Big Mac prices around 2018 to 2022. However, Turkey experienced a large drop in recent years while Russia experienced huge increase and than a large drop around 2015 and only had a smaller increase later on.
...
## 4.3 Adding GDP per capita to track the movement of BMI through the years and countries with a Bubble Plot
...
bmi_data %>%
  plot_ly(x = ~gdp_per_capita, 
          ...
          sizes = c(10, 200),
          frame = ~year, 
          text = ~paste(country, "<br>Big Mac Price: USD$", bmi_usd_price,"<br>GDP per Capita: $", gdp_per_capita, "<br>Inflation: ", inflation, "%"), 
          hoverinfo = "text",
          type = 'scatter',
          mode = 'markers'
          ) %>%
