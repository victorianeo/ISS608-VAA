...
#### Improving the visualisation of distribution in a boxplo
...
The code chunk below shows four different recommended ways to improve the boxplot's simplicity for 10 members of a group e.g. the top 5 and bottom 5 schools for reading scores.
...
read_counts <- selected_school_data %>%
  group_by(SCHOOLID) %>%
  summarise(count = n())
...
p3 <- p3 + 
   annotate("text", x = rep(0, nrow(read_counts)), y = read_counts$SCHOOLID, 
           label = paste("n =", read_counts$count),
           hjust = 1.2, vjust = 0.45, size = 3, color = "black")
